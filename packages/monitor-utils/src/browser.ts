...
import { StableTypes } from '@ny/monitor-shared';
  ...
  for (let i = 0, j = lines.length; i < j; ++i) {
    if ((parts = chrome.exec(lines[i]))) {
      ...
      element = {
        ...
        func: parts[1] || StableTypes.UNKNOWN_FUNCTION,
    ...
    if (!element.func && element.line) {
      element.func = StableTypes.UNKNOWN_FUNCTION;
    }
...
export function getDeviceInfo() {
  ...
  const { appCodeName, appName, appVersion, geolocation, language, onLine, platform, userAgent } = window.navigator;
  const deviceInfo = {
    appCodeName,
    appName,
    appVersion,
