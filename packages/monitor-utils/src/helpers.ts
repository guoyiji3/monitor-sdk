import { IAnyObject } from '@ny/monitor-types';
import { globalVar, ToStringTypes } from '@ny/monitor-shared';
import { logger } from './logger';
import { nativeToString, variableTypeDetection } from './is';
import { isWxMiniEnv, isBrowserEnv } from './global';
import { getCheckTime } from './time';
...
// 获取当前页面title
export function getLocationTitle(): string {
  if (typeof document === 'undefined' || document.title == null) return '';
...
export function toStringValidateOption(target: any, targetName: string, expectType: ToStringTypes): boolean {
  ...
  typeof target !== 'undefined' &&
    logger.error(`${targetName}期望传入:${expectType}类型，当前是:${nativeToString.call(target)}类型`);
  return false;
}
...
// 前进/后退按钮以及load事件触发后引起的pageshow事件
export function onBFCacheRestore(callback: (e: any) => void) {
...
// 关闭页面监听
export function onHidden(callback: (e: any) => void, once?: boolean) {
...
// 获取元素选择器
export const getSelectors = (pathsOrTarget: any) => {
